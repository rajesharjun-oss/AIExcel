...
auditRouter.post("/check", asyncRoute(async (req, res) => {
  ...
  try {
    const items = JSON.parse(raw) as Omit<AuditResult, "id">[];
    results = items.map((r, i) => ({ ...r, id: `ai-${Date.now()}-${i}` }));
  } catch {
    // model returned non-JSON — return empty results
  }
