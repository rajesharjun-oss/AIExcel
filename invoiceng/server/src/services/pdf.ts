...
export interface PdfReceiptData {
  ...
  readonly invoiceTotalKobo: number;
  readonly paidToDateKobo: number;
}

export function renderReceiptPdf(data: PdfReceiptData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: "A5", margin: 40, info: { Title: `Receipt ${data.receiptNumber}` } });
    const chunks: Buffer[] = [];
    ...
    doc.fillColor(BRAND).font("Helvetica-Bold").fontSize(18).text(data.businessName, 40, 40);
    if (data.businessTin) {
      doc.fillColor(SOFT).font("Helvetica").fontSize(8).text(`TIN: ${data.businessTin}`);
    }
    doc.moveDown(0.6);
    doc.fillColor(INK).font("Helvetica-Bold").fontSize(13).text("PAYMENT RECEIPT");
    doc.fillColor(SOFT).font("Helvetica").fontSize(9).text(data.receiptNumber);
    doc.moveDown(1);

    const rows: Array<[string, string]> = [
      ["Received from", data.customerName],
      ["Amount", ngn(data.amountKobo)],
      ["Method", data.method === "paystack" ? "Card / online (Paystack)" : data.method === "transfer" ? "Bank transfer" : "Cash"],
      ["Date", dateStr(data.paidAt)],
      ["For invoice", data.invoiceNumber],
      ["Invoice total", ngn(data.invoiceTotalKobo)],
      ["Paid to date", ngn(data.paidToDateKobo)],
      ["Balance", ngn(data.invoiceTotalKobo - data.paidToDateKobo)],
    ];
    let y = doc.y + 6;
    for (const [label, value] of rows) {
      doc.fillColor(SOFT).font("Helvetica").fontSize(9).text(label.toUpperCase(), 40, y, { width: w / 2 });
      doc
        .fillColor(INK)
        .font(label === "Amount" || label === "Balance" ? "Helvetica-Bold" : "Helvetica")
        .fontSize(label === "Amount" ? 13 : 10)
        .text(value, 40 + w / 2, y, { width: w / 2, align: "right" });
      y += label === "Amount" ? 22 : 18;
      doc.moveTo(40, y - 5).lineTo(40 + w, y - 5).strokeColor(LINE).stroke();
    }
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8)
      .text("Generated by InvoiceNG", 40, doc.page.height - 60, { width: w, align: "center" });
    doc.end();
  });
}
